...
print(f"max 점수: {sort_list[-1]}점, min 점수: {sort_list[0]}점")   
...
# 품목, 가격, 수량 튜플 만들기
newstocks = tuple(zip(*stocks))
print(newstocks[0])                                                    # ✅ ('사과', '바나나', '체리')
print(newstocks[1])                                                    # ✅ (1000, 2000, 5000)
print(newstocks[2])                                                    # ✅ (5, 3, 2)

total = sum(price * num for _, price, num in stocks)
print(f"총액: {total:,}원")

# 총 재고 금액 출력
cost = tuple(newstocks[1][x] * newstocks[2][x] for x in range(len(stocks)))
print(f"총액: {sum(cost):,}원")                                                    # ✅ 총액: 21,000원
